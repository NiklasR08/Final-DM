...
%%%%%%%%%%%%%%%%%%%%%%%%%
I don't know the position where they should be included but they are going to complete the analysis that you carried out so well.
POSSIBLE EXTENSIONS: 
1)relationship between HAPPINESS SCORE and region. Richest countries are happier?
if it is the case, then this could be explained by the fact that GDP weight is positive correlated with happiness
and Richer countries tends to value more GDP.

2) spend few words on the fact that economical and personal freedom are positive correlated, so that the two kind of freedom seems not to be in contrast with each other 
%%%%%%%%%%%%
...
Assess two kind of cluster analysis considering different variables.
#First one
The first one should consider only the columns of the first dataset (and only the weights, not the happiness score or any other variable ). In this way countries will be grouped by their system of values, countries that consider the same things as important will be together.
...
#On the elbow method
The optimal number of clusters should be the point (or close to it) where the total within variance starts decreasing more slowly (the descendant is less steeper). For example, this cleary occur with two nmber of clusters in your example.
