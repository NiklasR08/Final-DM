...
```{r}
ggplot(dt_ds, aes(Happiness.Score)) + geom_histogram(binwidth = 0.2)
```
```{r}
...
```{r}
par(mfrow = c(1, 3))
hist(dt_ds$pf_score, main = "Personal Freedom", xlab = "Score of Personal Freedom", xlim = c(1,10))
hist(dt_ds$ef_score, main = "Economic Freedom", xlab = "Score of Economic Freedom", xlim = c(1,10))
hist(dt_ds$hf_score, main = "Human Freedom", xlab = "Score of Human Freedom", xlim = c(1,10))
...
summary(dt_ds[,11:13])
```
The distribution of all three graphs is left-skewed. Based on the median we see no big difference between the economic freedom and personal freedom. By looking at the graphs on the other side we can see that personal freedom is more distributed between the maximum and minimum score compared to economic freedom which appears to be more centered around their median.
Remember that the human freedom score is the average on the pf_score and ef_score which we can see here (average pf_score + average ef_score = average hf_score).

**Economic Freedom**
```{r}
summary(dt_ds$ef_score)
```


**Human Freedom**
```{r}
summary(dt_ds$hf_score)
```
