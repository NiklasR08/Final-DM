...
```{r}
##MAYBE CHANGE THE VARIABLES WE USE FOR THE CLUSTERING
...
We will start by visualizing the optimal number of clusters:
```{r}
clus_dt_scaled <- scale(clus_dt)
rownames(clus_dt_scaled) = dt$countries
fviz_nbclust(clus_dt_scaled, kmeans, method = "wss", nstart = 200) +
  geom_vline(xintercept = 4, linetype = 2)
```
The graph suggests that the optimal number of cluster is probably between 4 and 5. Any number more will probably not add much values to the analysis and any number less still shows room for improvement. 
We will then proceed with the calculating the number of clusters using k-means.

```{r}
k = 4 # number of clusters
km_dt <- kmeans(clus_dt_scaled, k, nstart = 200)
print(km_dt)
```
We can see from this analysis which countries are in which cluster.
We can also calculate the average statistics of each cluster
```{r}
aggregate(clus_dt, by=list(cluster=km_dt$cluster), mean)
```


```{r}
fviz_cluster(km_dt, data = clus_dt_scaled, 
             ellipse.type = "euclid",
             star.plot = T,
             repel = T,
...
```{r}
fviz_nbclust(clus_dt_scaled, kmeans, method = "silhouette", nstart = 200)
```
```{r}
k = 2 # number of clusters
pam_dt = pam(clus_dt_scaled, k)
print(pam_dt)
```
```{r}
fviz_cluster(pam_dt, 
...
```
### 4.4 Hierachical Cluster
```{r}
dist.eucl = dist(clus_dt_scaled, method = "euclidean")
hc_dt = hclust(d = dist.eucl, method = "ward.D2")
fviz_dend(hc_dt, cex = 0.2)
```
**Cluster Tree Verification:**
```{r}
dt.coph = cophenetic(hc_dt)
cor(dist.eucl, dt.coph)
```
```{r}
dt.hc2 = hclust(d = dist.eucl, method = "average")
cor(dist.eucl, cophenetic(dt.hc2))
```
As the correlation is higher for the second model, this could be a sign for choosing the second method.
```{r}
fviz_dend(hc_dt, k = 2,
          cex = 0.3,
          ...
          rect = T)
```
### 4.5 Comparision of Clusters
In the following part we will look at the different clusters and compare the validity one the clusters.
