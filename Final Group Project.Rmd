...
```
We can see a pattern between the three clusters. Except for Corruption and Generosity the weight drops for each cluster. This means that countries in cluster one have on average the highest weight on all six variables that were used to calculate their Happiness Score. In the second cluster these values are lower than from countries from the first cluster but higher for countries from the third cluster (except for Corruption and Generosity). The third cluster group has the lowest weights on average for most of their variables. This is not the case for Corruption and Generosity, here they score higher than group two but lowter than group one.
...
### 4.4 Conclusion
We have the following conclusions for our clustering analysis
